...
/** Lazy view of `hord.types.v1.Events`: nested and repeated message fields are
...
pub struct EventsLazyView<'a> {
    ...
    pub heth_transfers: ::buffa::LazyRepeatedView<
        'a,
        super::super::__buffa::lazy_view::HethTransferLazyView<'a>,
    >,
    /// Field 2: `heth_staking_stats_updateds`
    pub heth_staking_stats_updateds: ::buffa::LazyRepeatedView<
        'a,
        super::super::__buffa::lazy_view::HethStakingStatsUpdatedLazyView<'a>,
    >,
}
        ...
        while !cur.is_empty() {
            ...
            match tag.field_number() {
                1u32 => {
                    ...
                    view.heth_transfers.push_bytes(sub, __sub_ctx);
                }
                2u32 => {
                    ::buffa::encoding::check_wire_type(
                        tag,
                    ...
                    let sub = ::buffa::types::borrow_bytes(&mut cur)?;
                    ctx.register_element_memory(::core::mem::size_of::<&'a [u8]>())?;
                    view.heth_staking_stats_updateds.push_bytes(sub, __sub_ctx);
                }
                _ => {
        ...
        ::core::result::Result::Ok(super::super::Events {
            heth_transfers: {
                let mut __out = ::buffa::alloc::vec::Vec::with_capacity(
                    self.heth_transfers.len(),
                );
                for __r in self.heth_transfers.iter() {
            ...
            heth_staking_stats_updateds: {
                let mut __out = ::buffa::alloc::vec::Vec::with_capacity(
                    self.heth_staking_stats_updateds.len(),
                );
                for __r in self.heth_staking_stats_updateds.iter() {
                    __out.push(__r?.to_owned_message()?);
                }
                __out
            },
...
impl<'a> EventsLazyView<'a> {
    ...
    pub fn compute_size(&self, _cache: &mut ::buffa::SizeCache) -> u32 {
        ...
        let mut size = 0u64;
        for __frag in self.heth_transfers.raw_elements() {
            size
                += 1u64 + ::buffa::encoding::varint_len(__frag.len() as u64) as u64
                    + __frag.len() as u64;
        }
        for __frag in self.heth_staking_stats_updateds.raw_elements() {
            size
                += 1u64 + ::buffa::encoding::varint_len(__frag.len() as u64) as u64
                    + __frag.len() as u64;
        }
...
/** Lazy view of `hord.types.v1.HethTransfer`: nested and repeated message fields are
 recorded as undecoded byte ranges and decoded on access. See
 [`::buffa::LazyMessageView`] for the deferred-validation contract;
 the eager, whole-tree-validated counterpart is named `HethTransferView`.

 Oneof variants, map values, groups, and extern-typed fields (e.g.
 well-known types) hold eagerly-decoded `HethTransferView`-family
 types; only singular/repeated message fields defer.
 ...
 let view = HethTransferLazyView::decode_lazy(&bytes)?;
 ```*/
#[derive(Clone, Debug, Default)]
pub struct HethTransferLazyView<'a> {
    /// Field 1: `id`
    pub id: &'a str,
    /// Field 2: `from`
    pub from: &'a str,
...
impl<'a> ::buffa::LazyMessageView<'a> for HethTransferLazyView<'a> {
...
impl<'a> ::buffa::MessageName for HethTransferLazyView<'a> {
    const PACKAGE: &'static str = "hord.types.v1";
...
/** Lazy view of `hord.types.v1.HethStakingStatsUpdated`: nested and repeated message fields are
 recorded as undecoded byte ranges and decoded on access. See
 [`::buffa::LazyMessageView`] for the deferred-validation contract;
 the eager, whole-tree-validated counterpart is named `HethStakingStatsUpdatedView`.

 Oneof variants, map values, groups, and extern-typed fields (e.g.
 well-known types) hold eagerly-decoded `HethStakingStatsUpdatedView`-family
 types; only singular/repeated message fields defer.
 ...
 let view = HethStakingStatsUpdatedLazyView::decode_lazy(&bytes)?;
 ```*/
#[derive(Clone, Debug, Default)]
pub struct HethStakingStatsUpdatedLazyView<'a> {
    /// Field 1: `id`
    pub id: &'a str,
...
impl<'a> HethStakingStatsUpdatedLazyView<'a> {
    /// Decode from `buf` under the limits carried by `ctx`, recording
    /// nested/repeated message fields as byte ranges.
    ///
    /// **Not part of the public API.**
...
impl<'a> ::buffa::LazyMessageView<'a> for HethStakingStatsUpdatedLazyView<'a> {
...
impl<'a> ::buffa::MessageName for HethStakingStatsUpdatedLazyView<'a> {
    const PACKAGE: &'static str = "hord.types.v1";
