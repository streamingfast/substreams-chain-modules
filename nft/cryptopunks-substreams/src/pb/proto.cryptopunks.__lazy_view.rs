...
/** Lazy view of `cryptopunks.types.v1.Trade`: nested and repeated message fields are
 recorded as undecoded byte ranges and decoded on access. See
 [`::buffa::LazyMessageView`] for the deferred-validation contract;
 the eager, whole-tree-validated counterpart is named `TradeView`.

 Oneof variants, map values, groups, and extern-typed fields (e.g.
 well-known types) hold eagerly-decoded `TradeView`-family
 ...
 let view = TradeLazyView::decode_lazy(&bytes)?;
 ```*/
#[derive(Clone, Debug, Default)]
pub struct TradeLazyView<'a> {
    /// Field 1: `id`
    pub id: &'a str,
    /// Field 2: `punk_index`
    pub punk_index: u64,
...
impl<'a> ::buffa::LazyMessageView<'a> for TradeLazyView<'a> {
...
impl<'a> ::buffa::MessageName for TradeLazyView<'a> {
...
/** Lazy view of `cryptopunks.types.v1.Bid`: nested and repeated message fields are
 recorded as undecoded byte ranges and decoded on access. See
 [`::buffa::LazyMessageView`] for the deferred-validation contract;
 the eager, whole-tree-validated counterpart is named `BidView`.

 Oneof variants, map values, groups, and extern-typed fields (e.g.
 well-known types) hold eagerly-decoded `BidView`-family
 types; only singular/repeated message fields defer.
 ...
 let view = BidLazyView::decode_lazy(&bytes)?;
 ```*/
#[derive(Clone, Debug, Default)]
pub struct BidLazyView<'a> {
    /// Field 1: `id`
    pub id: &'a str,
    /// Field 2: `punk_index`
    pub punk_index: u64,
...
impl<'a> ::buffa::LazyMessageView<'a> for BidLazyView<'a> {
...
impl<'a> ::buffa::MessageName for BidLazyView<'a> {
    const PACKAGE: &'static str = "cryptopunks.types.v1";
...
/** Lazy view of `cryptopunks.types.v1.Events`: nested and repeated message fields are
 recorded as undecoded byte ranges and decoded on access. See
 [`::buffa::LazyMessageView`] for the deferred-validation contract;
...
pub struct EventsLazyView<'a> {
    /// Field 1: `trades`
    pub trades: ::buffa::LazyRepeatedView<
        'a,
        super::super::__buffa::lazy_view::TradeLazyView<'a>,
    >,
    /// Field 2: `bids`
    pub bids: ::buffa::LazyRepeatedView<
        'a,
        super::super::__buffa::lazy_view::BidLazyView<'a>,
    >,
}
        ...
        while !cur.is_empty() {
            ...
            match tag.field_number() {
                1u32 => {
                    ...
                    view.trades.push_bytes(sub, __sub_ctx);
                }
                2u32 => {
                    ::buffa::encoding::check_wire_type(
                        tag,
                    ...
                    let sub = ::buffa::types::borrow_bytes(&mut cur)?;
                    ctx.register_element_memory(::core::mem::size_of::<&'a [u8]>())?;
                    view.bids.push_bytes(sub, __sub_ctx);
                }
                _ => {
                    ::buffa::encoding::skip_field_depth(tag, &mut cur, ctx.depth())?;
                }
    ...
    fn to_owned_message(
        ...
        ::core::result::Result::Ok(super::super::Events {
            trades: {
                ...
                for __r in self.trades.iter() {
                    __out.push(__r?.to_owned_message()?);
                }
                __out
            },
            bids: {
                let mut __out = ::buffa::alloc::vec::Vec::with_capacity(self.bids.len());
                for __r in self.bids.iter() {
                    __out.push(__r?.to_owned_message()?);
                }
                __out
            },
...
impl<'a> EventsLazyView<'a> {
    ...
    pub fn compute_size(&self, _cache: &mut ::buffa::SizeCache) -> u32 {
        ...
        use ::buffa::ViewEncode as _;
        let mut size = 0u64;
        for __frag in self.trades.raw_elements() {
            size
                += 1u64 + ::buffa::encoding::varint_len(__frag.len() as u64) as u64
                    + __frag.len() as u64;
        }
        for __frag in self.bids.raw_elements() {
            size
                += 1u64 + ::buffa::encoding::varint_len(__frag.len() as u64) as u64
                    + __frag.len() as u64;
        }
