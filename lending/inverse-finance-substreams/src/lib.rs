...
use crate::pb::inverse_finance::types::v1::{
    CTokenAccrueInterest, CTokenBorrow, CTokenLiquidateBorrow, CTokenMint, CTokenRedeem, CTokenRepayBorrow,
    DolaTransfer, Events, FactoryActionPaused, FactoryDistributedBorrowerComp, FactoryDistributedSupplierComp,
    FactoryMarketListed, FactoryNewCloseFactor, FactoryNewCollateralFactor, FactoryNewLiquidationIncentive,
    InvOwnerChanged, StablizerBuy, StablizerSell,
};
    ...
    for e in events.factory_new_liquidation_incentives {
        tables
            ...
            .set(
                ...
                &e.old_liquidation_incentive_mantissa,
            )
            .set(
                "new_liquidation_incentive_mantissa",
                &e.new_liquidation_incentive_mantissa,
            )
            .set("tx_hash", &e.tx_hash)
            .set("log_index", e.log_index as i64)
            .set("block_num", e.block_num as i64)
