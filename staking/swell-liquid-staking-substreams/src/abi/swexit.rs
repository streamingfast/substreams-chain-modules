// @generated
// This file was @generated by `substreams-ethereum-abigen`. Do not edit it by hand.

/// Contract's functions.
#[allow(dead_code, unused_imports, unused_variables)]
pub mod functions {}
/// Contract's events.
#[allow(dead_code, unused_imports, unused_variables)]
pub mod events {
    #[derive(Debug, Clone, PartialEq)]
    pub struct WithdrawRequestCreated {
        pub token_id: substreams::scalar::BigInt,
        pub amount: substreams::scalar::BigInt,
        pub timestamp: substreams::scalar::BigInt,
        pub last_token_id_processed: substreams::scalar::BigInt,
        pub rate_when_created: substreams::scalar::BigInt,
        pub owner: Vec<u8>,
    }
    impl WithdrawRequestCreated {
        const TOPIC_ID: [u8; 32] = [
            212u8,
            143u8,
            139u8,
            82u8,
            144u8,
            43u8,
            54u8,
            124u8,
            77u8,
            199u8,
            143u8,
            161u8,
            10u8,
            139u8,
            210u8,
            21u8,
            164u8,
            141u8,
            99u8,
            1u8,
            129u8,
            190u8,
            84u8,
            50u8,
            112u8,
            15u8,
            9u8,
            182u8,
            128u8,
            81u8,
            127u8,
            80u8,
        ];
        pub fn match_log<L: substreams_ethereum::LogLike>(log: &L) -> bool {
            if log.topic_count() != 3usize {
                return false;
            }
            if log.data().len() != 128usize {
                return false;
            }
            return log.topic(0).expect("bounds already checked") == Self::TOPIC_ID;
        }
        pub fn decode<L: substreams_ethereum::LogLike>(log: &L) -> Result<Self, String> {
            if log.topic_count() != 3usize {
                return Err(
                    format!(
                        "unexpected topic count, expected {}, got {}", 3usize, log
                        .topic_count()
                    ),
                );
            }
            if log.data().len() < 128usize {
                return Err(
                    format!(
                        "data too short, expected at least {}, got {}", 128usize, log
                        .data().len()
                    ),
                );
            }
            Ok(Self {
                last_token_id_processed: substreams_ethereum::abi::read_uint(
                    log.topic(1usize).expect("bounds already checked"),
                    0,
                    "last_token_id_processed",
                )?,
                owner: substreams_ethereum::abi::read_address(
                    log.topic(2usize).expect("bounds already checked"),
                    0,
                    "owner",
                )?,
                token_id: substreams_ethereum::abi::read_uint(
                    log.data(),
                    0,
                    "token_id",
                )?,
                amount: substreams_ethereum::abi::read_uint(log.data(), 32, "amount")?,
                timestamp: substreams_ethereum::abi::read_uint(
                    log.data(),
                    64,
                    "timestamp",
                )?,
                rate_when_created: substreams_ethereum::abi::read_uint(
                    log.data(),
                    96,
                    "rate_when_created",
                )?,
            })
        }
    }
    impl substreams_ethereum::Event for WithdrawRequestCreated {
        const NAME: &'static str = "WithdrawRequestCreated";
        fn match_log<L: substreams_ethereum::LogLike>(log: &L) -> bool {
            Self::match_log(log)
        }
        fn decode<L: substreams_ethereum::LogLike>(log: &L) -> Result<Self, String> {
            Self::decode(log)
        }
    }
    #[derive(Debug, Clone, PartialEq)]
    pub struct WithdrawalClaimed {
        pub owner: Vec<u8>,
        pub token_id: substreams::scalar::BigInt,
        pub exit_claimed_eth: substreams::scalar::BigInt,
    }
    impl WithdrawalClaimed {
        const TOPIC_ID: [u8; 32] = [
            45u8,
            67u8,
            235u8,
            23u8,
            71u8,
            135u8,
            21u8,
            81u8,
            50u8,
            181u8,
            45u8,
            219u8,
            107u8,
            52u8,
            110u8,
            45u8,
            202u8,
            153u8,
            48u8,
            46u8,
            172u8,
            61u8,
            244u8,
            70u8,
            109u8,
            190u8,
            255u8,
            149u8,
            61u8,
            60u8,
            132u8,
            209u8,
        ];
        pub fn match_log<L: substreams_ethereum::LogLike>(log: &L) -> bool {
            if log.topic_count() != 2usize {
                return false;
            }
            if log.data().len() != 64usize {
                return false;
            }
            return log.topic(0).expect("bounds already checked") == Self::TOPIC_ID;
        }
        pub fn decode<L: substreams_ethereum::LogLike>(log: &L) -> Result<Self, String> {
            if log.topic_count() != 2usize {
                return Err(
                    format!(
                        "unexpected topic count, expected {}, got {}", 2usize, log
                        .topic_count()
                    ),
                );
            }
            if log.data().len() < 64usize {
                return Err(
                    format!(
                        "data too short, expected at least {}, got {}", 64usize, log
                        .data().len()
                    ),
                );
            }
            Ok(Self {
                owner: substreams_ethereum::abi::read_address(
                    log.topic(1usize).expect("bounds already checked"),
                    0,
                    "owner",
                )?,
                token_id: substreams_ethereum::abi::read_uint(
                    log.data(),
                    0,
                    "token_id",
                )?,
                exit_claimed_eth: substreams_ethereum::abi::read_uint(
                    log.data(),
                    32,
                    "exit_claimed_eth",
                )?,
            })
        }
    }
    impl substreams_ethereum::Event for WithdrawalClaimed {
        const NAME: &'static str = "WithdrawalClaimed";
        fn match_log<L: substreams_ethereum::LogLike>(log: &L) -> bool {
            Self::match_log(log)
        }
        fn decode<L: substreams_ethereum::LogLike>(log: &L) -> Result<Self, String> {
            Self::decode(log)
        }
    }
}