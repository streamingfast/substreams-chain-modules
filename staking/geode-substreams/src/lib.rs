...
use crate::pb::geode::types::v1::{Events, GavaxTransferBatch, GavaxTransferSingle, PortalProposalApproved};
    ...
    for trx in block.transactions() {
        ...
        for log in receipt.logs.iter() {
            ...
            if log.address == PORTAL.as_slice() {
                if let Some(ev) = abi::portal::events::ProposalApproved::match_and_decode(&log) {
            ...
            if log.address == GAVAX.as_slice() {
                if let Some(ev) = abi::gavax::events::TransferSingle::match_and_decode(&log) {
                ...
                if let Some(ev) = abi::gavax::events::TransferBatch::match_and_decode(&log) {
