...
/** Lazy view of `deq.types.v1.Events`: nested and repeated message fields are
...
pub struct EventsLazyView<'a> {
    ...
    pub staked_avail_transfers: ::buffa::LazyRepeatedView<
        'a,
        super::super::__buffa::lazy_view::StakedAvailTransferLazyView<'a>,
    >,
}
        ...
        while !cur.is_empty() {
            ...
            match tag.field_number() {
                1u32 => {
                    ...
                    view.staked_avail_transfers.push_bytes(sub, __sub_ctx);
                }
                _ => {
        ...
        ::core::result::Result::Ok(super::super::Events {
            staked_avail_transfers: {
                let mut __out = ::buffa::alloc::vec::Vec::with_capacity(
                    self.staked_avail_transfers.len(),
                );
                for __r in self.staked_avail_transfers.iter() {
...
impl<'a> EventsLazyView<'a> {
    ...
    pub fn compute_size(&self, _cache: &mut ::buffa::SizeCache) -> u32 {
        ...
        let mut size = 0u64;
        for __frag in self.staked_avail_transfers.raw_elements() {
            size
...
/** Lazy view of `deq.types.v1.StakedAvailTransfer`: nested and repeated message fields are
 recorded as undecoded byte ranges and decoded on access. See
 [`::buffa::LazyMessageView`] for the deferred-validation contract;
 the eager, whole-tree-validated counterpart is named `StakedAvailTransferView`.

 Oneof variants, map values, groups, and extern-typed fields (e.g.
 well-known types) hold eagerly-decoded `StakedAvailTransferView`-family
 types; only singular/repeated message fields defer.
 ...
 let view = StakedAvailTransferLazyView::decode_lazy(&bytes)?;
 ```*/
#[derive(Clone, Debug, Default)]
pub struct StakedAvailTransferLazyView<'a> {
    /// Field 1: `id`
    pub id: &'a str,
    /// Field 2: `from`
    pub from: &'a str,
...
impl<'a> StakedAvailTransferLazyView<'a> {
    /// Decode from `buf` under the limits carried by `ctx`, recording
    /// nested/repeated message fields as byte ranges.
    ///
    /// **Not part of the public API.**
...
impl<'a> ::buffa::LazyMessageView<'a> for StakedAvailTransferLazyView<'a> {
...
impl<'a> ::buffa::MessageName for StakedAvailTransferLazyView<'a> {
    const PACKAGE: &'static str = "deq.types.v1";
