...
/** Lazy view of `aspida.types.v1.Events`: nested and repeated message fields are
...
pub struct EventsLazyView<'a> {
    ...
    pub aeth_transfers: ::buffa::LazyRepeatedView<
        'a,
        super::super::__buffa::lazy_view::AethTransferLazyView<'a>,
    >,
}
        ...
        while !cur.is_empty() {
            ...
            match tag.field_number() {
                1u32 => {
                    ...
                    view.aeth_transfers.push_bytes(sub, __sub_ctx);
                }
                _ => {
        ...
        ::core::result::Result::Ok(super::super::Events {
            aeth_transfers: {
                let mut __out = ::buffa::alloc::vec::Vec::with_capacity(
                    self.aeth_transfers.len(),
                );
                for __r in self.aeth_transfers.iter() {
...
impl<'a> EventsLazyView<'a> {
    ...
    pub fn compute_size(&self, _cache: &mut ::buffa::SizeCache) -> u32 {
        ...
        let mut size = 0u64;
        for __frag in self.aeth_transfers.raw_elements() {
            size
...
/** Lazy view of `aspida.types.v1.AethTransfer`: nested and repeated message fields are
 recorded as undecoded byte ranges and decoded on access. See
 [`::buffa::LazyMessageView`] for the deferred-validation contract;
 the eager, whole-tree-validated counterpart is named `AethTransferView`.

 Oneof variants, map values, groups, and extern-typed fields (e.g.
 well-known types) hold eagerly-decoded `AethTransferView`-family
 types; only singular/repeated message fields defer.
 ...
 let view = AethTransferLazyView::decode_lazy(&bytes)?;
 ```*/
#[derive(Clone, Debug, Default)]
pub struct AethTransferLazyView<'a> {
    /// Field 1: `id`
    pub id: &'a str,
    /// Field 2: `from`
    pub from: &'a str,
...
impl<'a> ::buffa::LazyMessageView<'a> for AethTransferLazyView<'a> {
...
impl<'a> ::buffa::MessageName for AethTransferLazyView<'a> {
    const PACKAGE: &'static str = "aspida.types.v1";
