// @generated
// This file was @generated by `substreams-ethereum-abigen`. Do not edit it by hand.

/// Contract's functions.
#[allow(dead_code, unused_imports, unused_variables)]
pub mod functions {}
/// Contract's events.
#[allow(dead_code, unused_imports, unused_variables)]
pub mod events {
    #[derive(Debug, Clone, PartialEq)]
    pub struct RewardVaultSet {
        pub old_reward_vault: Vec<u8>,
        pub new_reward_vault: Vec<u8>,
    }
    impl RewardVaultSet {
        const TOPIC_ID: [u8; 32] = [
            93u8,
            165u8,
            71u8,
            185u8,
            209u8,
            111u8,
            223u8,
            36u8,
            210u8,
            239u8,
            252u8,
            90u8,
            171u8,
            85u8,
            37u8,
            223u8,
            2u8,
            181u8,
            67u8,
            200u8,
            247u8,
            43u8,
            157u8,
            236u8,
            56u8,
            103u8,
            204u8,
            129u8,
            171u8,
            163u8,
            89u8,
            130u8,
        ];
        pub fn match_log<L: substreams_ethereum::LogLike>(log: &L) -> bool {
            if log.topic_count() != 3usize {
                return false;
            }
            if log.data().len() != 0usize {
                return false;
            }
            return log.topic(0).expect("bounds already checked") == Self::TOPIC_ID;
        }
        pub fn decode<L: substreams_ethereum::LogLike>(log: &L) -> Result<Self, String> {
            if log.topic_count() != 3usize {
                return Err(
                    format!(
                        "unexpected topic count, expected {}, got {}", 3usize, log
                        .topic_count()
                    ),
                );
            }
            Ok(Self {
                old_reward_vault: substreams_ethereum::abi::read_address(
                    log.topic(1usize).expect("bounds already checked"),
                    0,
                    "old_reward_vault",
                )?,
                new_reward_vault: substreams_ethereum::abi::read_address(
                    log.topic(2usize).expect("bounds already checked"),
                    0,
                    "new_reward_vault",
                )?,
            })
        }
    }
    impl substreams_ethereum::Event for RewardVaultSet {
        const NAME: &'static str = "RewardVaultSet";
        fn match_log<L: substreams_ethereum::LogLike>(log: &L) -> bool {
            Self::match_log(log)
        }
        fn decode<L: substreams_ethereum::LogLike>(log: &L) -> Result<Self, String> {
            Self::decode(log)
        }
    }
    #[derive(Debug, Clone, PartialEq)]
    pub struct Staked {
        pub staker: Vec<u8>,
        pub amount: substreams::scalar::BigInt,
        pub new_stake: substreams::scalar::BigInt,
        pub new_total_principal: substreams::scalar::BigInt,
    }
    impl Staked {
        const TOPIC_ID: [u8; 32] = [
            180u8,
            202u8,
            175u8,
            41u8,
            173u8,
            218u8,
            62u8,
            239u8,
            238u8,
            58u8,
            213u8,
            82u8,
            168u8,
            232u8,
            80u8,
            88u8,
            88u8,
            155u8,
            248u8,
            52u8,
            199u8,
            70u8,
            108u8,
            174u8,
            78u8,
            229u8,
            135u8,
            135u8,
            247u8,
            5u8,
            137u8,
            237u8,
        ];
        pub fn match_log<L: substreams_ethereum::LogLike>(log: &L) -> bool {
            if log.topic_count() != 2usize {
                return false;
            }
            if log.data().len() != 96usize {
                return false;
            }
            return log.topic(0).expect("bounds already checked") == Self::TOPIC_ID;
        }
        pub fn decode<L: substreams_ethereum::LogLike>(log: &L) -> Result<Self, String> {
            if log.topic_count() != 2usize {
                return Err(
                    format!(
                        "unexpected topic count, expected {}, got {}", 2usize, log
                        .topic_count()
                    ),
                );
            }
            if log.data().len() < 96usize {
                return Err(
                    format!(
                        "data too short, expected at least {}, got {}", 96usize, log
                        .data().len()
                    ),
                );
            }
            Ok(Self {
                staker: substreams_ethereum::abi::read_address(
                    log.topic(1usize).expect("bounds already checked"),
                    0,
                    "staker",
                )?,
                amount: substreams_ethereum::abi::read_uint(log.data(), 0, "amount")?,
                new_stake: substreams_ethereum::abi::read_uint(
                    log.data(),
                    32,
                    "new_stake",
                )?,
                new_total_principal: substreams_ethereum::abi::read_uint(
                    log.data(),
                    64,
                    "new_total_principal",
                )?,
            })
        }
    }
    impl substreams_ethereum::Event for Staked {
        const NAME: &'static str = "Staked";
        fn match_log<L: substreams_ethereum::LogLike>(log: &L) -> bool {
            Self::match_log(log)
        }
        fn decode<L: substreams_ethereum::LogLike>(log: &L) -> Result<Self, String> {
            Self::decode(log)
        }
    }
    #[derive(Debug, Clone, PartialEq)]
    pub struct Unstaked {
        pub staker: Vec<u8>,
        pub amount: substreams::scalar::BigInt,
        pub new_stake: substreams::scalar::BigInt,
        pub new_total_principal: substreams::scalar::BigInt,
    }
    impl Unstaked {
        const TOPIC_ID: [u8; 32] = [
            32u8,
            79u8,
            204u8,
            240u8,
            217u8,
            46u8,
            216u8,
            212u8,
            143u8,
            32u8,
            74u8,
            219u8,
            57u8,
            178u8,
            232u8,
            30u8,
            146u8,
            186u8,
            208u8,
            222u8,
            219u8,
            147u8,
            245u8,
            113u8,
            108u8,
            169u8,
            71u8,
            140u8,
            251u8,
            87u8,
            222u8,
            0u8,
        ];
        pub fn match_log<L: substreams_ethereum::LogLike>(log: &L) -> bool {
            if log.topic_count() != 2usize {
                return false;
            }
            if log.data().len() != 96usize {
                return false;
            }
            return log.topic(0).expect("bounds already checked") == Self::TOPIC_ID;
        }
        pub fn decode<L: substreams_ethereum::LogLike>(log: &L) -> Result<Self, String> {
            if log.topic_count() != 2usize {
                return Err(
                    format!(
                        "unexpected topic count, expected {}, got {}", 2usize, log
                        .topic_count()
                    ),
                );
            }
            if log.data().len() < 96usize {
                return Err(
                    format!(
                        "data too short, expected at least {}, got {}", 96usize, log
                        .data().len()
                    ),
                );
            }
            Ok(Self {
                staker: substreams_ethereum::abi::read_address(
                    log.topic(1usize).expect("bounds already checked"),
                    0,
                    "staker",
                )?,
                amount: substreams_ethereum::abi::read_uint(log.data(), 0, "amount")?,
                new_stake: substreams_ethereum::abi::read_uint(
                    log.data(),
                    32,
                    "new_stake",
                )?,
                new_total_principal: substreams_ethereum::abi::read_uint(
                    log.data(),
                    64,
                    "new_total_principal",
                )?,
            })
        }
    }
    impl substreams_ethereum::Event for Unstaked {
        const NAME: &'static str = "Unstaked";
        fn match_log<L: substreams_ethereum::LogLike>(log: &L) -> bool {
            Self::match_log(log)
        }
        fn decode<L: substreams_ethereum::LogLike>(log: &L) -> Result<Self, String> {
            Self::decode(log)
        }
    }
}