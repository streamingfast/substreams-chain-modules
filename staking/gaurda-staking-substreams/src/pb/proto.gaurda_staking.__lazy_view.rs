...
/** Lazy view of `gaurda_staking.types.v1.Events`: nested and repeated message fields are
...
pub struct EventsLazyView<'a> {
    ...
    pub geth_transfers: ::buffa::LazyRepeatedView<
        'a,
        super::super::__buffa::lazy_view::GethTransferLazyView<'a>,
    >,
}
        ...
        while !cur.is_empty() {
            ...
            match tag.field_number() {
                1u32 => {
                    ...
                    view.geth_transfers.push_bytes(sub, __sub_ctx);
                }
                _ => {
        ...
        ::core::result::Result::Ok(super::super::Events {
            geth_transfers: {
                let mut __out = ::buffa::alloc::vec::Vec::with_capacity(
                    self.geth_transfers.len(),
                );
                for __r in self.geth_transfers.iter() {
...
impl<'a> EventsLazyView<'a> {
    ...
    pub fn compute_size(&self, _cache: &mut ::buffa::SizeCache) -> u32 {
        ...
        let mut size = 0u64;
        for __frag in self.geth_transfers.raw_elements() {
            size
...
/** Lazy view of `gaurda_staking.types.v1.GethTransfer`: nested and repeated message fields are
 recorded as undecoded byte ranges and decoded on access. See
 [`::buffa::LazyMessageView`] for the deferred-validation contract;
 the eager, whole-tree-validated counterpart is named `GethTransferView`.

 Oneof variants, map values, groups, and extern-typed fields (e.g.
 well-known types) hold eagerly-decoded `GethTransferView`-family
 types; only singular/repeated message fields defer.
 ...
 let view = GethTransferLazyView::decode_lazy(&bytes)?;
 ```*/
#[derive(Clone, Debug, Default)]
pub struct GethTransferLazyView<'a> {
    /// Field 1: `id`
    pub id: &'a str,
    /// Field 2: `from`
    pub from: &'a str,
...
impl<'a> ::buffa::LazyMessageView<'a> for GethTransferLazyView<'a> {
...
impl<'a> ::buffa::MessageName for GethTransferLazyView<'a> {
    const PACKAGE: &'static str = "gaurda_staking.types.v1";
