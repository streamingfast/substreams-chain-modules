...
use crate::pb::tether_gold::types::v1::{Events, XautMint, XautRedeem, XautTransfer};
    ...
    for trx in block.transactions() {
        ...
        for log in trx.receipt().logs() {
            ...
            if log.address() == XAUT.as_slice() {
                if let Some(ev) = abi::xaut::events::Transfer::match_and_decode(log) {
                ...
                if let Some(ev) = abi::xaut::events::Mint::match_and_decode(log) {
                ...
                if let Some(ev) = abi::xaut::events::Redeem::match_and_decode(log) {
