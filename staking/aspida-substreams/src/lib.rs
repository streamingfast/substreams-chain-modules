...
use crate::pb::aspida::types::v1::{AethTransfer, Events};

const AETH: [u8; 20] = hex_literal::hex!("fc87753df5ef5c368b5fba8d4c5043b77e8c5b39");
...
pub fn map_events(block: &BlockLazyView<'_>) -> Result<Events, Error> {
    ...
    for trx in block.transactions() {
        ...
        for lc in trx.logs_with_calls()? {
            ...
            if log.address == AETH {
                if let Some(ev) = abi::aeth::events::Transfer::match_and_decode(&log) {
                    events.aeth_transfers.push(AethTransfer {
                        id: id.clone(),
...
pub fn db_out(events: Events) -> Result<DatabaseChanges, Error> {
    ...
    for e in events.aeth_transfers {
        tables
            .create_row("aeth_transfer", &e.id)
            .set("from", &e.from)
            .set("to", &e.to)
            .set("value", &e.value)
