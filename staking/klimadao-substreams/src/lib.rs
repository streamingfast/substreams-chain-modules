...
pub fn db_out(events: Events) -> Result<DatabaseChanges, Error> {
    ...
    for e in events.bct_transfers {
        tables
            .create_row("bct_transfer", &e.id)
            .set("from", &e.from)
            .set("to", &e.to)
            .set("value", &e.value)
    ...
    for e in events.bct_redeem_fee_paids {
        tables
            .create_row("bct_redeem_fee_paid", &e.id)
