...
/** Lazy view of `inception.types.v1.Events`: nested and repeated message fields are
...
pub struct EventsLazyView<'a> {
    ...
    pub ineth_transfers: ::buffa::LazyRepeatedView<
        'a,
        super::super::__buffa::lazy_view::InethTransferLazyView<'a>,
    >,
}
        ...
        while !cur.is_empty() {
            ...
            match tag.field_number() {
                1u32 => {
                    ...
                    view.ineth_transfers.push_bytes(sub, __sub_ctx);
                }
                _ => {
        ...
        ::core::result::Result::Ok(super::super::Events {
            ineth_transfers: {
                let mut __out = ::buffa::alloc::vec::Vec::with_capacity(
                    self.ineth_transfers.len(),
                );
                for __r in self.ineth_transfers.iter() {
...
impl<'a> EventsLazyView<'a> {
    ...
    pub fn compute_size(&self, _cache: &mut ::buffa::SizeCache) -> u32 {
        ...
        let mut size = 0u64;
        for __frag in self.ineth_transfers.raw_elements() {
            size
...
/** Lazy view of `inception.types.v1.InethTransfer`: nested and repeated message fields are
 recorded as undecoded byte ranges and decoded on access. See
 [`::buffa::LazyMessageView`] for the deferred-validation contract;
 the eager, whole-tree-validated counterpart is named `InethTransferView`.

 Oneof variants, map values, groups, and extern-typed fields (e.g.
 well-known types) hold eagerly-decoded `InethTransferView`-family
 types; only singular/repeated message fields defer.
 ...
 let view = InethTransferLazyView::decode_lazy(&bytes)?;
 ```*/
#[derive(Clone, Debug, Default)]
pub struct InethTransferLazyView<'a> {
    /// Field 1: `id`
    pub id: &'a str,
    /// Field 2: `from`
    pub from: &'a str,
...
impl<'a> InethTransferLazyView<'a> {
    /// Decode from `buf` under the limits carried by `ctx`, recording
    /// nested/repeated message fields as byte ranges.
    ///
    /// **Not part of the public API.**
...
impl<'a> ::buffa::LazyMessageView<'a> for InethTransferLazyView<'a> {
...
impl<'a> ::buffa::MessageName for InethTransferLazyView<'a> {
    const PACKAGE: &'static str = "inception.types.v1";
